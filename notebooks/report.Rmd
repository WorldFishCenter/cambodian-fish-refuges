...
add_fitted_draws(data_frame(species_type_simple = unique(msc$data$species_type_simple)),
                 msc, re_formula = NA) %>%
  ungroup() %>%
  ...
  stat_pointinterval() +
  geom_text(data = labels, aes(label = label, x = x, y = y, hjust = hjust), fontface = "italic") +
  theme_minimal() +
  labs(x = "t-value", 
       y = "Species type", 
       tag = "b)")
...
gather_draws(tbi_model, `b_.*`,  regex = T) %>%
  ...
  mutate(base_level = find_base_level(variable, level, tbi_model$data),
         ...
         response = case_when(response == "b_bstnd_" ~ "Species Loss", 
                              TRUE ~ "Species Gains")) %>%
  ggplot(aes(x = .value, y = var_level)) +
  ...
  geom_text(aes(label = pd, x = pd_position), stat = "unique", 
            hjust = "outward", size = 3.2, colour = "grey30") +
